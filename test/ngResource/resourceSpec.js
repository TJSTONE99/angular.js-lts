...
describe('resource', () => {
  ...
  describe('basic usage', () => {
    let $resource, CreditCard, callback, $httpBackend, resourceProvider, $q;

    beforeEach(angular.mock.module('ngResource'));

    beforeEach(angular.mock.module($resourceProvider => {
    ...
    describe('shallow copy', () => {
      ...
      it('should make a copy', () => {
        const original = { key: {} };
        const copy = ngInternals.shallowClearAndCopy(original);
    ...
    it('should send correct headers', () => {
      $httpBackend.expectPUT('/CreditCard/123', undefined, headers => {
    ...
    it('should support dynamic default parameters (global)', () => {
      const currentGroup = 'students', Person = $resource('/Person/:group/:id', { group: function () { return currentGroup; } });
    ...
    describe('promise api', () => {
      ...
      describe('single resource', () => {
        ...
        it('should allow promise chaining', () => {
          ...
          cc.$promise.then(value => { return 'new value'; }).then(callback);
          $httpBackend.flush();
        ...
        it('should allow $promise error callback registration', () => {
          ...
          const response = callback.mock.calls[callback.mock.calls.length - 1][0];
        ...
        it('should allow parsing a value from headers', () => {
          ...
          const parseUrlFromHeaders = response => {
      ...
      describe('resource collection', () => {
        ...
        it('should allow promise chaining', () => {
          ...
          ccs.$promise.then(value => { return 'new value'; }).then(callback);
          $httpBackend.flush();
    ...
    describe('success mode', () => {
      it('should call the success callback (as 1st argument) on 2xx responses', () => {
        let instance, headers, status, statusText;
        const successCb = jest.fn((d, h, s, t) => {
  ...
  describe('extra params', () => {
    ...
    beforeEach(angular.mock.inject((_$http_, _$httpBackend_, _$resource_, _$rootScope_) => {
      $http = _$http_;
  ...
  describe('handling rejections', () => {
    ...
    beforeEach(angular.mock.inject((_$exceptionHandler_, _$httpBackend_, _$resource_) => {
    ...
    it('should not report a PUR when an error callback or responseError interceptor is provided',
      () => {
        ...
        // With error callback
        CreditCard.test1(angular.noop, noop);
        $httpBackend.flush();
        ...
        CreditCard.test2(angular.noop, noop);
        $httpBackend.flush();
    ...
    it('should report a PUR when the responseError interceptor returns a rejected promise',
      angular.mock.inject($q => {
        ...
        expect($exceptionHandler.errors.length).toBe(1);
        expect($exceptionHandler.errors[0].toString()).toMatch(/^Possibly unhandled rejection/);
      })
    );
    ...
    it('should not propagate exceptions in success callback to the returned promise', () => {
      const successCallbackSpy = jest.fn().mockImplementation(() => {
        throw new Error('error')
      });
      const promiseResolveSpy = jest.fn();
    ...
    it('should not be able to recover from inside the error callback', () => {
      const errorCallbackSpy = jest.fn().mockReturnValue({ id: 123 });
      const promiseResolveSpy = jest.fn();
      const promiseRejectSpy = jest.fn();
    ...
    describe('requestInterceptor', () => {
      const rejectReason = { 'lol': 'cat' };
      let $q, $rootScope;
      let successSpy, failureSpy, callback;
  ...
  describe('cancelling requests', () => {
    ...
    beforeEach(angular.mock.module('ngResource', $provide => {
      $provide.decorator('$http', $delegate => {
    ...
    beforeEach(angular.mock.inject((_$httpBackend_, _$resource_, _$timeout_) => {
      $httpBackend = _$httpBackend_;
      $resource = _$resource_;
      $timeout = _$timeout_;
    }));
    ...
    it('should use `cancellable` value if passed a non-numeric `timeout` in an action',
      angular.mock.inject(($log, $q, $rootScope) => {
        jest.spyOn($log, 'debug').mockImplementation(() => { });
        $httpBackend.whenGET('/CreditCard').respond({});
  ...
  describe('configuring `cancellable` on the provider', () => {
    ...
    beforeEach(angular.mock.module('ngResource', $resourceProvider => {
      $resourceProvider.defaults.cancellable = true;
    }));

    beforeEach(angular.mock.inject(_$resource_ => {
      $resource = _$resource_;
    }));
