...
describe('q', () => {
  let q, q_no_error, defer, deferred, promise, log, exceptionHandlerCalls;
  ...
  function success(name, returnVal, throwReturnVal) {
    const returnValDefined = (arguments.length >= 2);
    name = 'success' + (name || '');
    return function () {
  ...
  var mockNextTick = {
    ...
    flush: function () {
      ...
      while (mockNextTick.queue.length) {
        const queue = mockNextTick.queue;
        mockNextTick.queue = [];
        angular.forEach(queue, task => {
          try {
            task();
          } catch (e) {
  ...
  describe('$Q', () => {
    let resolve, reject, resolve2, reject2;
    const createPromise = () => {
    ...
    it('should support the instanceof operator', () => {
      ...
      let promise = new q(angular.noop);
  ...
  describe('all (array)', () => {
    ...
    it('should take an array of promises and return a promise for an array of results', () => {
      const deferred1 = defer(), deferred2 = defer();
  ...
  describe('when exceptionHandler is called', () => {
    ...
    const errorStr = ngInternals.toDebugString(errorEg);

    const customError = new CustomError('Custom');
    const customErrorStr = ngInternals.toDebugString(customError);

    const nonErrorObj = { isATest: 'this is' };
    const nonErrorObjStr = ngInternals.toDebugString(nonErrorObj);
    ...
    angular.forEach(fixtures, fixture => {
