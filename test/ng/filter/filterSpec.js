...
describe('Filter: filter', () => {
  ...
  it('should take function as predicate', () => {
    ...
    expect(filter(items, i => { return i.done; }).length).toBe(1);
  });
  ...
  it('should pass the index to a function predicate', () => {
    ...
    const result = filter(items, (value, index) => {
      return index % 2 === 0;
    });
  ...
  it('should ignore function properties in expression', () => {
    ...
    let expr = { text: 'hello', func: angular.noop };
  ...
  it('should not throw an error if used with an array like object', () => {
    ...
    const nodeList = angular.element('<p><span>Misko</span><span>Igor</span><span>Brad</span></p>')[0].childNodes;
